...
- Damage grouping — hits landing close together form one group, each new number taking the next spot in a fixed alternating pattern (middle, right, left, further out) around the impact. The pattern never varies, so the same burst always builds the same recognisable shape, and numbers are free to overlap where they cross — nothing pushes them apart. Numbers already in a group flinch when a new hit lands, so a sustained burst reads as damage piling up. Spacing and size hold the same on screen at any range, and the oldest numbers fade out early once the screen is carrying too many
